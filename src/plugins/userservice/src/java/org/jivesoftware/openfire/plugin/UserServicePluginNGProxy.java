...
public class UserServicePluginNGProxy implements UserServiceManager {
	...
	public void createUser(UserEntity userEntity) throws ServiceException {
		...
		try {
			VCardManager.getInstance().setVCard(userEntity.getUsername(), getDefaultVCard());
		} catch (Exception e) {
			throw new ServiceException(COULD_NOT_CREATE_VCARD, "",
					COULD_NOT_CREATE_VCARD,
		...
		RosterItemEntity rosterOfNewUser = createRosterEntity(
				userEntity.getUsername()
						...
						+ XMPPServer.getInstance().getServerInfo()
								.getXMPPDomain(), userEntity.getName(),
				fromOfSubscriptionLevel);
		
		addRosterItem(realOfUserServiceManager, masterId,
				rosterOfNewUser);
	}
	...
	private void addRosterItem(UserServiceManager realOfUserServiceManager,
			String username, RosterItemEntity rosterOfMaster) throws ServiceException {
		try {
			realOfUserServiceManager.addRosterItem(username,
					rosterOfMaster);
